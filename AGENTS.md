...
- **LLM prompts are the behavior spec.** `llm.py` contains three system prompts that define how the bot understands messages. Changing these changes bot behavior — treat them as the most sensitive code in the repo.
...
Requires a `.env` file (copy from `.env.example`) and `credentials/service_account.json` for Google Sheets export.

## Where Behavior Lives

| What to change | File | Notes |
|---|---|---|
| Bot intent recognition | `llm.py` → `_CLASSIFIER_SYSTEM` | Three intents: `balance`, `split_change`, `expense` |
...
- Google Sheets export uses `value_input_option="USER_ENTERED"` so formulas in the sheet would be evaluated.
